...
class MainView extends HookWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final _deck = useState<List<int>>(List.generate(textsProvider.texts.length, (index) => index));
    ...
    useEffect(updateDeck, [_bookmarkFilter.value,]);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            icon: Icon([Icons.circle_outlined,Icons.question_mark,Icons.keyboard][mode.value]),
          ),
          IconButton(
            onPressed: () {
          ...
          IconButton(
            ...
            onPressed: () => showPagePicker(context, _currentCard.value, (nextCard) {
      ...
      body: PageView.builder(
        ...
        itemBuilder: (BuildContext context, int index) => index >= _deck.value.length || index < 0 ? Container() : CardView(_deck.value[index],mode.value),
      ),
    );
  }
}
