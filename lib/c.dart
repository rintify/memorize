...
class Chars {
  ...
  Chars(String script) {
    ...
    for (var rune in script.runes) {
      ...
      if (modeHurigana) {
        ...
        modeBlank = false;
        blanks.last.end = text.length;
      } else if (c == '\n') {
        lines.add(text.length);
      } else if (c == '<') {
        modeBlank = true;
        blanks.add(Blank(id: blanks.length, start: text.length));
      } else if (c == '#') {
        modeTag = true;
        segments.last.tags.add('');
      } else if (c == '{') {
        modeHurigana = true;
        huriganas.add(Hurigana(start: lastNoReqHurigana + 1, end:  text.length, text: ''));
        lastNoReqHurigana = text.length - 1;
      } else if (c == '/') {
        segments.add(Segment(segments.length,text.length));
      } else if (rune < 0x20) {
      } else {
  ...
  String toScript(){
    ...
    for(final segment in segments){
      (insert[segment.start] ??= []).add('${segment.start == 0 ? '' : '/'}${segment.tags.map((e)=>'#$e ').join('')}');
    }
    ...
    return insertAtPositions(text, insert);
  }

  String insertAtPositions(String input, Map<int, List<String>> insert) {
...
class Blank extends Range{
  ...
  Blank({this.id = 0, int start = 0, int end = 0}) : super(start: start, end: end);
}

class Hurigana extends Range {
  String text;

  Hurigana({this.text = '', int start = 0, int end = 0}) : super(start: start, end: end);
}
...
Widget CharsView(Chars cs, {Widget Function(Chars cs, int pos) cview = noqchar}) {
  final List<Range> viewLines = [];
  int lineI = 0;
  int pos = cs.lines[lineI];
  for(;;){
    final int end = lineI + 1 == cs.lines.length ? cs.text.length : cs.lines[lineI + 1];
    final pre = pos;
    pos += 26;
    if(pos >= end){
      pos = end;
      ...
      for (final viewLine in viewLines)
        Padding(
          ...
          child: Column(
            ...
            children: [for(var i = viewLine.start; i < viewLine.end; i ++) cview(cs,i)
            ],
          ),
        ),
    ],
...
Widget noqchar(Chars cs, int pos) {
  ...
  if (hurigana == null) {
    return character(cs.text[pos], style);
  } else {
    return Stack(
      clipBehavior: Clip.none,
      children: [
        character(cs.text[pos], style),
        Positioned(
          // ここでポップアップの位置を調整
          ...
          child: Material(
            ...
            child: Column(
              mainAxisSize: MainAxisSize.min,
              mainAxisAlignment: MainAxisAlignment.center,
              children: List.from(hurigana.text.characters.map((c)=>character(c, hstyle))),
            ),
          ),
        ),
      ],
